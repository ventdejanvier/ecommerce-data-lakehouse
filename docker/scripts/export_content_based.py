...
def first_existing_column(df, candidates: tuple[str, ...], role: str) -> str:
    ...
    raise ValueError(
        f"gold_db.recommendations_content_based is missing a column for {role}. "
...
def export_content_based_recommendations():
    ...
    recommended_col = first_existing_column(
        gold_recommendations,
        ("recommended_product_id", "similar_product_id", "target_product_id", "product_id_2"),
        "recommended product",
    )
    score_col = first_existing_column(
        gold_recommendations,
        ("score", "content_score", "similarity_score", "cosine_similarity"),
        "score",
    )
    ...
    recommendations = gold_recommendations \
        .select(
            ...
            F.col(recommended_col).cast("string").alias("recommended_product_id"),
